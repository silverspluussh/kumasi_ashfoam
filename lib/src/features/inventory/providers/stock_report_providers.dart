...
final stockReportsListProvider = FutureProvider<List<StockReportSummary>>((ref) async {
  final dbService = ref.watch(databaseServiceProvider);
  final reports = await dbService.getStockReports();
  return reports.map((r) => StockReportSummary.fromMap(r.toMap())).toList();
});

/// Provider for monthly date range filter [start, end]
/// Format: DateTime(year, month)
final stockReportFilterProvider = StateProvider<List<DateTime?>>((ref) => [null, null]);

/// Computed provider for filtered stock reports
final filteredStockReportsProvider = Provider<AsyncValue<List<StockReportSummary>>>((ref) {
  ...
  return reportsAsync.whenData((reports) {
    ...
    return reports.where((report) {
      final reportDate = DateTime(report.createdAt.year, report.createdAt.month);
      
      bool isAfterStart = true;
      if (start != null) {
        isAfterStart = reportDate.isAtSameMomentAs(start) || reportDate.isAfter(start);
      }
      
      bool isBeforeEnd = true;
      if (end != null) {
        isBeforeEnd = reportDate.isAtSameMomentAs(end) || reportDate.isBefore(end);
      }
